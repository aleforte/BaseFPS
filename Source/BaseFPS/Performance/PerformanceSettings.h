...
#include "PerformanceStatTypes.h"
...
class UPlatformSpecificRenderingSettings : public UPlatformSettings
{
	GENERATED_BODY()

public:
	UPlatformSpecificRenderingSettings();

	// Helper method to get the performance settings object, directed via platform settings
	static const UPlatformSpecificRenderingSettings* Get();
 ...
 */
UCLASS(config=Game, defaultconfig, meta=(DisplayName="Performance Settings"))
class BASEFPS_API UPerformanceSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()
		
public:
	UPerformanceSettings();
...
public:
	...
	TArray<FPerformanceStatGroup> UserFacingPerformanceStats;
};
