...
#include "BaseFPSInputModifier.h"

#include "EnhancedPlayerInput.h"
#include "AimSensitivityData.h"
#include "Player/BaseFPSLocalPlayer.h"

DEFINE_LOG_CATEGORY_STATIC(LogBaseFPSInputModifiers, Log, All);

/************************************************************************/
/* BaseFPSputModifiersHelpers                                           */
/************************************************************************/

namespace BaseFPSInputModifiersHelpers
{
	/** Returns the owning CommonLocalPlayer of an Enhanced Player Input pointer */
	static UBaseFPSLocalPlayer* GetLocalPlayer(const UEnhancedPlayerInput* PlayerInput)
	{
		if (PlayerInput)
		{
			if (APlayerController* PC = Cast<APlayerController>(PlayerInput->GetOuter()))
			{
				return Cast<UBaseFPSLocalPlayer>(PC->GetLocalPlayer());
			}
		}
		return nullptr;
	}
...
/************************************************************************/
/* UBaseFPSSettingBasedScalar                                           */
/************************************************************************/

FInputActionValue UBaseFPSInputModifier::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput,
	FInputActionValue CurrentValue, float DeltaTime)
{
	...
	{
		if (UBaseFPSLocalPlayer* LocalPlayer = BaseFPSInputModifiersHelpers::GetLocalPlayer(PlayerInput))
		{
			const UClass* SettingsClass = UBaseFPSSettingsShared::StaticClass();
			UBaseFPSSettingsShared* SharedSettings = LocalPlayer->GetSharedSettings();
...
/************************************************************************/
/* UBaseFPSInputModifierDeadZone                                        */
/************************************************************************/

FInputActionValue UBaseFPSInputModifierDeadZone::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput,
	FInputActionValue CurrentValue, float DeltaTime)
{
	EInputActionValueType ValueType = CurrentValue.GetValueType();
	UBaseFPSLocalPlayer* LocalPlayer = BaseFPSInputModifiersHelpers::GetLocalPlayer(PlayerInput);
	if (ValueType == EInputActionValueType::Boolean || !LocalPlayer)
	{
		return CurrentValue;
	}
	
	UBaseFPSSettingsShared* Settings = LocalPlayer->GetSharedSettings();
	ensure(Settings);
...
FLinearColor UBaseFPSInputModifierDeadZone::GetVisualizationColor_Implementation(FInputActionValue SampleValue,
...
/************************************************************************/
/* UBaseFPSInputModifierGamepadSensitivity                              */
/************************************************************************/

FInputActionValue UInputModifierGamepadSensitivity::ModifyRaw_Implementation(
	const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	...
	const EGamepadSensitivity Sensitivity = (TargetingType == EBaseFPSTargetingType::Normal) ? Settings->GetGamepadLookSensitivityPreset() : Settings->GetGamepadTargetingSensitivityPreset();
...
/************************************************************************/
/* UInputModifierAimInversion                                           */
/************************************************************************/

FInputActionValue UInputModifierAimInversion::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput,
                                                                            FInputActionValue CurrentValue, float DeltaTime)
{
	UBaseFPSLocalPlayer* LocalPlayer = BaseFPSInputModifiersHelpers::GetLocalPlayer(PlayerInput);
	if (!LocalPlayer)
